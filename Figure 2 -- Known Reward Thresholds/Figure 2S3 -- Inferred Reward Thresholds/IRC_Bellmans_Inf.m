...
% inferred reward change task with infinite m_r from 
% Barendregt et al., 2022.

function [thresh,rho] = IRC_Bellmans_Inf(x_r,h_r,T,dt,t_i,dg,m,c,R_H,R_L)
...
% Calculate exact reward timeseries from Markov process realization:
g_r = 0.5*(x_r+1);
R_t = NaN(1,length(x_r));
R_t(x_r == 1) = R_H; R_t(x_r == -1) = R_L;
...
[V(:,end),V_I(:,end)] = max([g'*R_t(end)-t_i*rho(k) (1-g)'*R_t(end)-t_i*rho(k)],[],2);
for j = (length(t)-1):-1:1
    [V(:,j),V_I(:,j)] = max([g'*R_t(j)-t_i*rho(k) (1-g)'*R_t(j)-t_i*rho(k) P_gg*(V_H(:,j+1)*(P_HH*g_r(j)+P_LH*(1-g_r(j)))+V_L(:,j+1)*(P_HL*g_r(j)+P_LL*(1-g_r(j))))-c(t(j))*dt-rho(k)*dt],[],2);
end
