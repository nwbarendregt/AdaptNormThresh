...
% continuous 2AFC tasks with changing SNR given by Eq. (8) in 
% Barendregt et al., 2022.

function [y,p]= SC_RDMD_trial_generate(m,T,dt,sigma,N_trial)
...
for i = 1:N_trial
    for j = 2:(T/dt+1)
        ...
        y(i,j) = y(i,j-1)+m(j-1)*dt+sqrt(2*m(j-1))*dW+sigma*randn;
        y_p(i,j) = y(i,j-1)+m(j-1)*dt+sqrt(2*m(j-1))*dW;
    end
end
