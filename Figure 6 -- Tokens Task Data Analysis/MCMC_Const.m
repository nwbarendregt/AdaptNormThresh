% MCMC_Const.m
% Evolves parameter chains for Const model using MCMC from 
% Barendregt et al., 2022.

function [theta,L] = MCMC_Const(theta_p,L_p,Nt,Sub_T,stim,S,prior)

% Draw proposed new state from Gaussian proposal distribution:
theta = mvnrnd(theta_p,S);
if (theta(1) > prior(1)) && (theta(1) < prior(2)) &&...
        (theta(2) > prior(3)) && (theta(3) > prior(4)) % Reject new state if parameters are negative.
