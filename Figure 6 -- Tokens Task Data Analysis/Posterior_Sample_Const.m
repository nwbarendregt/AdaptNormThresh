% Posterior_Sample_Const.m
% Performs MCMC sampling used for fitting Const model to tokens task data 
...
post_samp = NaN(N_samples/N_thin,3,N_chain);
...
for i = 1:N_chain
    post_samp_i = NaN(N_samples/N_thin,3);

    % Seed chains with final states and likelihoods from burn-in:
    theta = model_fit_Const(sub_ind,speed_ind).chain_init(:,i);
    L = model_fit_Const(sub_ind,speed_ind).L_init(i);
...
% comparison, etc.):
% load('model_fit_Const.mat')
% model_fit_Const(sub_ind,speed_ind).samples = post_samp;
